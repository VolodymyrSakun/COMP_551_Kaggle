...
algorithms = ['SVClinears', 'SVC', 'RF', 'XG', 'LR', 'NB']
#Load images with numpy
imagesTrain = np.load('train_imagesBW30x30Clean.npy', encoding='latin1')
imagesForecast = np.load('test_imagesBW30x30Clean.npy', encoding='latin1')
...
print("Logistic Regression") # only 1 core
classifierLR = LogisticRegression(penalty='l2', dual=False, tol=0.00001, C=1,\
    fit_intercept=False, intercept_scaling=1, class_weight='balanced', \
    solver='lbfgs', max_iter=10000, multi_class='ovr', \
    warm_start=False, n_jobs=1)
    
scoreLR = validateFit(classifierLR, dataTrain, columnsTrain, 'Y', nFolds=5, Labels=None)

y_LR = fitPredict(classifierLR, dataTrain, dataForecast, columnsTrain, 'Y')
labelsLR = list(labelencoder_X.inverse_transform(y_LR))
labelsForecast['LR'] = labelsLR
...
summary.loc['LR', 'F1_Average'] = scoreLR['F1_Average']
summary.loc['LR', 'Accuracy'] = scoreLR['Accuracy']
